---
name: adapter-design
description: 外界と接続するアダプタの実装指針。インバウンド(HTTP view / 管理コマンド / イベントハンドラ / 定期実行)が同じ usecase を呼ぶときの共通化と分担、アウトバウンド(外部 API クライアント / 通知 / ファイル)の作り方、リトライ・タイムアウト・サーキットブレーカをアダプタ内に閉じる判断、外部形式を自分の型へ翻訳する位置、認証と認可をどの層で行うか、実物を叩かずにアダプタをテストする方法を扱う。外部 API クライアントを書くとき、cron や CLI から usecase を呼びたいとき、リトライをどこに置くか迷うとき、認可チェックの置き場所を決めるときに使う。
allowed-tools:
  - Read
  - Grep
  - Glob
