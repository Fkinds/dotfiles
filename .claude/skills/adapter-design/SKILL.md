---
name: adapter-design
description: 外界と接続するアダプタの実装指針。外部 API クライアントを書くとき、cron や CLI から usecase を呼びたいとき、リトライをどこに置くか迷うとき、テストや開発で本物の API を叩きたくないとき、認可チェックの置き場所を決めるときに使う。扱う範囲はインバウンド(HTTP view / 管理コマンド / イベントハンドラ / 定期実行)が同じ usecase を呼ぶときの共通化と分担、アウトバウンド(外部 API クライアント / 通知 / ファイル)の作り方、1 ポートに Real / Fake / Noop の 3 実装を持つ構成、リトライ・タイムアウト・サーキットブレーカをアダプタ内に閉じる判断、外部形式を自分の型へ翻訳する位置、認証と認可をどの層で行うか、実物を叩かずにアダプタをテストする方法。
---
...
**1 つのポートに実装を 3 つ用意する。** 本物を叩く Real、副作用なしで検証できる Fake、
何もしない Noop。どれを使うかは設定で決め、**選択と import は composition root に閉じる**
— そうしないとアプリの import グラフに本番アダプタが残り、テストでも SDK 初期化や
認証情報の要求が走る。切り替えの実装は
