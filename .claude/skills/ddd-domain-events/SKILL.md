...
description: ドメインイベントの設計指針。「起きた事実」としてのイベント定義、集約ルートでの発生と収集、トランザクション確定後のディスパッチ、集約をまたぐ結果整合性、ハンドラの冪等性、outbox パターンを扱う。1 トランザクションで複数の集約を更新したくなったとき、副作用(通知・在庫引当・監査ログ)をドメインの振る舞いから切り離したいとき、ドメインイベントを設計・レビューするときに使う。
allowed-tools:
  - Read
  - Grep
  - Glob
