...
description: SOLID 原則をコードの判断に落とし込む指針。クラスや関数が肥大化して責務が混ざってきたとき、種類ごとの if 分岐が増えるとき、継承するか合成するか迷うとき、抽象やモックが多すぎてテストが読めないとき、SOLID の観点でレビューするときに使う。扱う範囲は単一責任(変更理由で測る)、開放閉鎖(ABC/Protocol による拡張点)、リスコフの置換(継承より合成、Django モデル継承の落とし穴)、インターフェース分離(太った抽象の分割)、依存性逆転(抽象への依存と注入)を、Python / Django の具体例と検出方法つきで。
allowed-tools:
  - Read
  - Grep
  - Glob
