...
description: モジュール・パッケージ間の依存関係を設計する指針。何を同じモジュールに入れるかの凝集性(再利用/変更理由/共倒れ)、循環依存の検出と 4 通りの解消法、安定度と抽象度の対応(依存は安定した方向へ)、Django app 間の依存とレイヤ違反の検出、依存グラフの測り方を扱う。循環 import が出たとき、モジュール分割やディレクトリ構成を決めるとき、どの app にコードを置くか迷うとき、変更のたびに無関係なモジュールが壊れるときに使う。
allowed-tools:
  - Read
  - Grep
  - Glob
