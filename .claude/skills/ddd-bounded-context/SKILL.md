...
description: 戦略的設計の指針。ユビキタス言語、コアドメイン/支援/汎用サブドメインの見極めと投資判断、境界づけられたコンテキストの引き方、コンテキストマップの関係パターン(共有カーネル・顧客/供給者・順応者・腐敗防止層・公開ホストサービス)、コンテキスト間の統合とモデルの翻訳、Django プロジェクトへの落とし込みを扱う。同じ言葉が部署によって違う意味で使われているとき、モデルが肥大化して一貫した定義ができないとき、外部システムや別チームのモデルと統合するとき、どこに設計コストをかけるか決めるときに使う。
allowed-tools:
  - Read
  - Grep
  - Glob
